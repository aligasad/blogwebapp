...
function AddUserBlog() {
  ...
  const addUserBlog = async (e) => {
    e.preventDefault();
    try {
      const user = JSON.parse(localStorage.getItem("user"));
      if (!user || !user.user?.uid) {
        alert("⚠️ Please login first to add your blog.");
        return;
      }
      ...
      const blogData = {
        ...
        userId: user.user.uid, // ✅ store logged-in user ID
        approved: false, // optional: for admin approval system
      };
  ...
  return (
    <div className="py-6 min-h-screen bg-gradient-to-br from-green-50 to-lime-100 flex items-center justify-center">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl p-8 border border-green-100">
        ...
        <form onSubmit={addUserBlog} className="grid grid-cols-1 gap-4">
          <input
            type="text"
            name="title"
